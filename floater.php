<!-- <div
  class="a2a_kit a2a_kit_size_36 a2a_floating_style a2a_vertical_style"
  data-a2a-scroll-show="80,200"
  style="left:0px; top:50%;transform:translateY(-50%);"
>
  <a
    class="a2a_button_facebook"
    href="https://www.facebook.com/pcet.punebschool"
  ></a>
  <a class="a2a_button_twitter"></a>
  <a class="a2a_button_google_gmail" href="mailto:punebschool@gmail.com"></a>
  <a class="a2a_button_linkedin"></a>
</div> -->
<div class="social-icons" id="social">
  <ul>
    <li>
      <a
        href="https://www.facebook.com/pcet.punebschool"
        target="_blank"
        class="marginleft10"
        ><img alt="" src="http://www.pccoepune.com/images/fb.png"
      /></a>
    </li>
    <li>
      <a
        href="https://www.linkedin.com/company/pccoe-pune?trk=biz-companies-cym"
        target="_blank"
        ><img
          class="marginleft10"
          src="http://www.pccoepune.com/images/Linkedin-icon.png"
      /></a>
    </li>
    <li>
      <a href="https://twitter.com/school_pune" target="_blank"
        ><img
          alt="PCCOE is the best Engineering Institute in Pimpri Chinchwad Pune."
          class="marginleft10"
          src="http://www.pccoepune.com/images/twitter.png"
      /></a>
    </li>
    <li>
      <a href="https://www.instagram.com/punebschool/" target="_blank"
        ><img
          alt="PCCOE, the famous Engineering College in Pune provides the placement in the most reputed companies."
          class="marginleft10"
          src="http://www.pccoepune.com/images/insta-icon.png"
          width="32px"
          height="31px"
      /></a>
    </li>
    <li>
      <a href="mailto:info@punebusinessschool.com" target="_blank"
        ><img
          class="marginleft10"
          src="http://www.pccoepune.com/images/Google-plus-icon.png"
      /></a>
    </li>
  </ul>
</div>
